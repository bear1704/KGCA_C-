...
struct VS_INPUT
{
	...
	float4 weight1 :	 TEXCOORD1;
	float4 idx2 :		 TEXCOORD1;
	float4 weight2 :	 TEXCOORD1;
};
...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	float4 vtx_local = float4(input.p, 1.0f);
	float4 vtx_anim;
	...
	for (int biped = 0; biped < 8; biped++)
	{
		...
		{
			uint idx_bone = input.index1[biped];
		...
		{
			uint idx_bone = input.index1[biped - 4];
			float weight = input.weight1[biped - 4];
			mat_anim = g_mat_anim[idx_bone];
			vtx_anim += weight * mul(vtx_local, mat_anim);
			pOut.normal += weight * mul(input.normal, (float3x3)mat_anim);
		}
	...
	float dot = max(0.0f, dot(vec_light, pOut.normal));
	float4 vec_color = float4(dot, dot, dot, 1.0f);
	pOut.r = vec_color;
	pOut.color = vec_color;
...
float4 PS(VS_OUTPUT input) : SV_TARGET
{
	...
	float dot = max(0.33f, dot(vec_light, input.normal));
	float4 vec_color = float4(dot, dot, dot, 1.0f);
	float4 color = vec_color * g_tex_diffuse.Sample(s0, input.tex);

	return color * float(input.color.xyz, 1.0f);
}
