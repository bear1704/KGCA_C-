...
void PObjectRotateUtil::RotateAndDraw(PSprite* sprite, int image_number ,float posX, float posY, float angle)
{
	...
	GetRotationBitmap(&one_frame_sprite, colorDC, angle , new_size, new_size, true);

	BLENDFUNCTION bf;
	bf.BlendOp = AC_SRC_OVER;
	bf.BlendFlags = 0;
	bf.SourceConstantAlpha = 1.0f * 255.f;
...
void PObjectRotateUtil::GetRotationBitmap(PSprite* one_frame_sprite, HDC& colorDC, float angle, float width, float height, bool clockwise)
{
	...
	RECT sprite_rect = one_frame_sprite->get_rect_list_copy()[0];
		...
		xform.eDx = (sprite_rect.right / 2);
		xform.eDy = (sprite_rect.bottom / 2);
		...
		//StretchBlt(destDC, -(sprite_rect.right / 2),
		//	-(sprite_rect.bottom / 2),
		//	sprite_rect.right, sprite_rect.bottom, srcDC, sprite_rect.left, sprite_rect.top, sprite_rect.right, sprite_rect.bottom, SRCCOPY);
