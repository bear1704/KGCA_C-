...
std::list<UPACKET> PNetwork::recv_pool_;
std::list<UPACKET> PNetwork::send_pool_;
...
bool PNetwork::SendFromPacketPool()
{
	if (PNetwork::connect_ == false) return true;
	
	for (UPACKET& packet : PNetwork::send_pool_)
	{
...
const SOCKET& PNetwork::get_socket()
{
	return socket_;
}
...
std::list<UPACKET>& PNetwork::get_recv_pool()
{
	return recv_pool_;
}
