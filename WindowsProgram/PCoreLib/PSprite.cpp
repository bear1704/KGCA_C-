...
bool PSprite::Alpha24BitsDraw(PSprite sprite, float alpha, float scale, HDC colorDC, HDC maskDC)
{
	...
	//DOUBLE_RECT rectangle_client_for_idiot;
	...
	//배경은 늘어난 크기만큼 잘라와 줘야 한다. 또한 센터 출력이므로 센터 - DOUBLE_RECT/2만큼의 위치부터 가져와야 한다. 
...
vector<DOUBLE_RECT> PSprite::get_rect_list_copy()
{
	vector<DOUBLE_RECT> copyrect;
